...
def ff_ind49_download():
    ...
    res = []
    with zipfile.open('Siccodes49.txt') as f:
    ...
    zipfile.close()
    df = (
        pd.DataFrame(res, columns=['ind49', 'sic1', 'sic2', 'ind_name'])
        .sort_values(['ind49', 'sic1'], ignore_index=True))

    df.to_parquet(download_dir/'ff_ind49.parquet.gzip', compression='gzip')
